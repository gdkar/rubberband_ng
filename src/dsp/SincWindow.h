...
public:
     ...
     */
    SincWindow(size_t n, size_t p) : m_size(n), m_p(p), m_cache(0) {encache();}
     ...
     */
    inline void rewrite(size_t p) {
        if (m_p == p) return;
        m_p = p;
        encache();
    }
    inline void cut(T *const dst) const {v_multiply(dst, m_cache, m_size);}
    ...
    inline T at(off_t i) const { return m_cache[i]; }
     ...
     */
    static
    void write(T *const dst, const size_t n, const size_t p) {
        const auto half = n/2;
        writeHalf(dst, n, p);
        auto target = half - 1;
        for (auto i = half + 1; i < n; ++i) {dst[target--] = dst[i];}
...
protected:
     ...
     */
    static
    void writeHalf(T *const dst, const size_t n, const size_t p) {
        const auto half = n/2;
        const T twopi = 2. * M_PI;
        dst[half] = T(1.0);
        for (auto i = decltype(half){1}; i < half; ++i) {
            T arg = T(i) * twopi / p;
    ...
    void encache() {
        ...
        m_area = 0;
        for (auto i = decltype(m_size){0}; i < m_size; ++i) {m_area += m_cache[i];}
        m_area /= m_size;
    }
};
